            ...
            <div class="col-12">
                ...
                <div class="card">
                    <div class="card-header">
                        <a href="{{ url('admin/post/create') }}" class="btn btn-default">Add New</a>
                        ...
                        <table class="table table-hover">
                            ...
                            @foreach( $data as $key => $d )
                                <tr style="font-style: {{ $d['status'] == 0 ? ' italic':'' }}">
                                    ...
                                    <td>{{ $d['author'] }}</td>
                                    <td>
                                        @for($i=0; $i<=count($d['category']) - 1; $i++)
                                            @if(count($d['category']) - 1 == $i)
                                                {{ $d['category'][$i] }}
                                            @else
                                                {{ $d['category'][$i] }},
                                            @endif
                                    ...
                                    <td>{{ $d['created_at'] }}</td>
                                    <td class="no">
                                        <a href="{{ url('admin/post/'.$d['id'].'/edit') }}" title="Edit">
                                            <i class="fa fa-edit"></i>
                                        </a>

                                        <form style="display: inline" action="{{ url('admin/post/'.$d['id'])}}"
                                              method="post">
